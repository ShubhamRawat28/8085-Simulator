...
string execution(string command,string  accumulator,string Registers[], bool flag[], map<string,string>&memory,string programCounter){
	...
	if(commandPart[0] == "MOV"){
			...
			commandSize = opcode(commandPart[0]);
			...
			ADD(commandPart[1],accumulator,Registers,flag,memory);
			commandSize = opcode(commandPart[0]);
			return nextAddress(programCounter,commandSize);
	}
			...
			SUB(commandPart[1],accumulator,Registers,flag,memory);
			commandSize = opcode(commandPart[0]);
			return nextAddress(programCounter,commandSize);
	}
	else if(commandPart[0] == "SUI"){

			SUI(commandPart[1],accumulator,Registers,flag);
			commandSize = opcode(commandPart[0]);
			return nextAddress(programCounter,commandSize);
	}
	else if(commandPart[0] == "INR"){

			INR(commandPart[1],Registers,accumulator,flag,memory);
			commandSize = opcode(commandPart[0]);
			return nextAddress(programCounter,commandSize);
	}
	else if(commandPart[0] == "DCR"){

			DCR(commandPart[1],Registers,accumulator,flag,memory);
			commandSize = opcode(commandPart[0]);
			return nextAddress(programCounter,commandSize);
	}
